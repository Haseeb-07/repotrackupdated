    ...
    <div className="p-4">
      <div className="flex justify-between mb-4">
        <div className='flex gap-6' >
          <button className="bg-green-500 dark:text-white py-2 px-4 rounded-full">Client details</button>
          <button className="bg-darkBlue dark:text-white py-2 px-4 rounded-full">Agent details</button>
        </div>
        <div className="flex space-x-4">
          <div className="relative group">
            <button className="dark:bg-darkBlue dark:text-white py-2 px-4 rounded-full">Compare</button>
            <div className="absolute right-0 mt-2 w-48 dark:bg-white border rounded shadow-xl hidden group-hover:block">
          ...
          <div className="relative group">
            <button className="dark:bg-darkBlue dark:text-white py-2 px-4 rounded-full">Date Filter</button>
            <div className="absolute right-0 mt-2 w-48 dark:bg-white border rounded shadow-xl hidden group-hover:block">
      ...
      <h2 className="dark:text-white mb-4">Client Details In Last 30 Days</h2>

      <div className="overflow-x-auto">
        <table className="min-w-full dark:bg-darkBlue">
          <thead>
          ...
          <tbody>
            {currentRows.map((row) => (
              <tr key={row.id} className="bg-myTable dark:text-white">
      ...
      <div className="flex justify-between items-center mt-4">
        <div className="flex">
          <button
            ...
            className={`px-3 py-1 rounded ${currentPage === 1 ? 'bg-gray-600' : 'bg-darkBlue dark:text-white'}`}
          >
            Prev
          </button>
          {[...Array(totalPages)].map((_, index) => (
            <button
              ...
              className={`px-3 py-1 rounded ${currentPage === index + 1 ? 'bg-red-500 dark:text-white' : 'bg-darkBlue dark:text-white'}`}
            >
              {index + 1}
            </button>
          ))}
          <button
            ...
            className={`px-3 py-1 rounded ${currentPage === totalPages ? 'bg-gray-600' : 'bg-darkBlue dark:text-white'}`}
          >
            Next
          </button>
        </div>
